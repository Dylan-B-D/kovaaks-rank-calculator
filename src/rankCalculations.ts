...
export function calculateOverallRank(
	...
	const rankCalculators: Record<
		string,
		(apiData: BenchmarkApiData, config: Difficulty) => RankCalculationResult
	> = {
		...
		dojo2: calculateDojo2Rank,
		'mira-apex': calculateMiraApexRank,
		rbe: calculateRbeRank,
		routine: calculateRoutineRank
	};

	const calculator =
		rankCalculators[benchmark.rankCalculation] ||
		(() => ({
	...
	// Special case for generic-energy which needs all difficulties
	const {
		rank: overallRank,
		details,
		fallbackUsed = false
		...
		: benchmark.rankCalculation === 'generic-energy-uncapped'
			...
			: benchmark.rankCalculation === 'ra-s5'
				...
				: benchmark.rankCalculation === 'jade-palace-ground'
					? calculateJadePalaceGroundRank(apiData, difficultyConfig, benchmark.difficulties)
...
/**
 * Calculates Routine rank
 * Requires at least half or more of scenarios to be a rank or higher
 */
function calculateRoutineRank(
	apiData: BenchmarkApiData,
): RankCalculationResult {
	if (!apiData.categories) return { rank: 0, details: {} };

	// 1. Collect all scenario ranks and count total scenarios
	...
	if (totalScenarios === 0) {
		return { rank: 0, details: { totalScenarios: 0, required: 0, progressToNextRank: 0 } };
	}

	// 2. Calculate required count (at least half)
	// "at least half or more" means >= ceil(total / 2)
	// e.g. 5 scenarios -> need 3. 4 -> need 2.
	const requiredCount = Math.ceil(totalScenarios / 2);

	// 3. Count occurrences of each rank
	const rankCounts = scenarioRanks.reduce<Record<number, number>>(
		(acc, rank) => ((acc[rank] = (acc[rank] || 0) + 1), acc),
		{}
	);

	// 4. Determine highest rank that meets the requirement
	const ranks = Object.keys(rankCounts)
		.map(Number)
		.sort((a, b) => b - a);
	let currentRank = 0;

	// Check cumulative counts from highest rank downwards
	for (const rank of ranks) {
		// Sum of all scenarios at this rank or higher
		const countAtOrAbove = ranks
			.filter((r) => r >= rank)
			.reduce((sum, r) => sum + (rankCounts[r] || 0), 0);

		if (countAtOrAbove >= requiredCount) {
			currentRank = rank;
			break;
		}
	}
	...
	// If we're at the highest possible rank, progress should be 100%
	const maxPossibleRank = Math.max(...ranks, 0);
	if (currentRank >= maxPossibleRank) {
		return {
			rank: currentRank,
			details: {
				totalScenarios,
	...
	const countAtNextOrAbove = ranks
		.filter((r) => r >= nextRank)
		.reduce((sum, r) => sum + (rankCounts[r] || 0), 0);
	
	const progressToNextRank = Math.min(countAtNextOrAbove / requiredCount, 1);
...
/**
 * Calculates RBE rank
 * Requires 9 scenarios at a rank to achieve that rank
 */
function calculateRbeRank(
	apiData: BenchmarkApiData,
	_difficultyConfig: Difficulty
): RankCalculationResult {
	if (!apiData.categories) return { rank: 0, details: {} };
	...
	// Find the highest rank where we have at least 9 scenarios at that rank or higher (cumulative)
	let finalRank = 0;
	...
	if (finalRank > 0) {
		...
		const countAtNextOrHigher = sortedRanks
			.filter((r) => r >= nextRank)
			.reduce((sum, r) => sum + rankCounts[r], 0);
		progressToNextRank = Math.min(countAtNextOrHigher / 9, 1);
	}
 ...
 */
function calculateMiraApexRank(
	apiData: BenchmarkApiData,
	...
	difficultyConfig.categories.forEach((categoryConfig) => {
		...
		categoryConfig.subcategories.forEach((subcategoryConfig) => {
			...
			for (
				let i = 0;
				i < subcategoryConfig.scenarioCount && scenarioIndex < scenarioNames.length;
				i++, scenarioIndex++
			) {
				...
				if (score > 0 && scenarioData.rank_maxes && scenarioData.rank_maxes.length > 0) {
					...
					if (rankInfo.baseRank === 0) {
						...
						const upper =
							rankThresholds[rankInfo.baseRank] || rankThresholds[rankThresholds.length - 1];
 ...
 */
function calculateCompleteOnlyRank(
	apiData: BenchmarkApiData,
	difficultyConfig: Difficulty
): RankCalculationResult {
...
/**
 * Calculates Dojo2 rank
 * Requires 3 scenarios at a rank to achieve that rank
 * Scores can be in any category or subcategory
 */
function calculateDojo2Rank(
	apiData: BenchmarkApiData,
	_difficultyConfig: Difficulty
): RankCalculationResult {
	if (!apiData.categories) return { rank: 0, details: {} };
	...
	// Find the highest rank where we have at least 3 scenarios at that rank or higher (cumulative)
	let finalRank = 0;
	const sortedRanks = Object.keys(rankCounts)
		.map(Number)
		.sort((a, b) => b - a); // Sort descending
	...
	if (finalRank > 0) {
		...
		const countAtNextOrHigher = sortedRanks
			.filter((r) => r >= nextRank)
			.reduce((sum, r) => sum + rankCounts[r], 0);
		progressToNextRank = Math.min(countAtNextOrHigher / 3, 1);
	}
...
function calculateDeadmanStaticRank(
			...
			for (
				...
				if (score > 0) {
					...
					if (rankInfo.baseRank === 0) {
						...
						const secondHighestRankMaxScore =
							scenarioData.rank_maxes.length > 1
	...
	if (
		validEnergies.length === expectedSubcategoryCount &&
...
export function calculateRaS5Rank(
	...
	const energyThresholds = getDifficultyThresholdSlice(
		globalThresholds,
		difficultyConfig,
		allDifficulties
	);
			...
			for (
				...
				if (rankInfo.baseRank === 0) {
					...
					if (adjustedFirst > 0) {
						...
						const perc = frac * 100;
						percentage =
							Math.round(perc * Math.pow(10, roundDecimals)) / Math.pow(10, roundDecimals);
					}
			...
			if (subcategoryScenarioEnergies.length === 0) {
				...
				if (subcategoryScenarioEnergies.length !== 4) {
					...
					const groundMax = Math.max(
						subcategoryScenarioEnergies[0],
						subcategoryScenarioEnergies[1]
					);
...
function calculateMiyuRank(
	...
	const progressToNextRank =
		rank >= thresholds.length ? 1 : calculateProgressToNextRank(totalPoints, thresholds, rank);
 ...
 */
function calculateDarkRafalRank(
	apiData: BenchmarkApiData,
	difficultyConfig: Difficulty
): RankCalculationResult {
	if (!apiData.categories) return { rank: 0, details: {} };
...
function calculateAoiRank(
	...
	for (const category of difficultyConfig.categories) {
		for (const sub of category.subcategories) {
			...
			for (
				let i = 0;
				i < sub.scenarioCount && scenarioIndex < scenarioNames.length;
				i++, scenarioIndex++
			) {
	...
	const allRanks = Array.from(new Set(subcategoryRanks.flat()))
		.filter((r) => r > 0)
		.sort((a, b) => b - a);

	let finalRank = 0;
	for (const rank of allRanks) {
		const oneScore = subcategoryRanks.filter(
			(ranks) => ranks.filter((r) => r >= rank).length >= 1
		).length;
		const twoScores = subcategoryRanks.filter(
			(ranks) => ranks.filter((r) => r >= rank).length >= 2
		).length;
	...
	if (finalRank > 0) {
		...
		if (finalRank >= maxRank) {
			...
			const oneScore = subcategoryRanks.filter(
				(ranks) => ranks.filter((r) => r >= next).length >= 1
			).length;
			const twoScores = subcategoryRanks.filter(
				(ranks) => ranks.filter((r) => r >= next).length >= 2
			).length;
...
function calculateTskRank(
	...
	const scenarios = Object.values(apiData.categories).flatMap((category) =>
		Object.values(category.scenarios).map((scenario) => {
			...
			const isOverMax =
				rankInfo.isMaxed &&
				rankInfo.baseRank === maxRank &&
				actualScore > scenario.rank_maxes[maxRank - 1];
			return {
	...
	// check for max rank scenarios
		...
		const maxRankScenarios = scenarios.filter(
			(s) => s.rankMaxes.length > 0 && s.rank === s.rankMaxes.length
		);
	...
	try {
		harmonicMean = Math.trunc(
			calculateHarmonicMean(
				scenarioEnergies.filter((e) => e > 0),
				scenarioEnergies.length
			)
		);
	} catch {}

	return calculateRankTemplate(
		apiData,
		difficultyConfig,
		() => ({
 ...
 */
function calculateUnofficialSas2Rank(
	apiData: BenchmarkApiData,
	difficultyConfig: Difficulty
): RankCalculationResult {
	const sas2Config = {
 ...
 */
function calculateMiraRank(
	apiData: BenchmarkApiData,
	difficultyConfig: Difficulty
): RankCalculationResult {
	const isEasy = difficultyConfig.difficultyName.toLowerCase() === 'easy';
	const thresholds = isEasy ? [100, 200, 300, 400, 500] : [600, 700, 800, 900, 1000];
	const miraConfig = {
		thresholds,
		fakeLowerOffset: 50,
		fakeUpperCount: 1
 ...
 */
function calculateVtEnergyRank(
	apiData: BenchmarkApiData,
	difficultyConfig: Difficulty
): RankCalculationResult {
...
function calculateCbS1Rank(
	...
	const { results } = processScenarios(
		...
		(_, __, scenarioName, scenarioData) => {
			const actualScore = convertApiScore(scenarioData.score);
			const { baseRank, isMaxed, progressToNext } = calculatePreciseRankFromScore(
				actualScore,
				scenarioData.rank_maxes
			);
			...
			if (baseRank === 0) {
				...
				scenarioWeight =
					weights[baseRank - 1] + (weights[baseRank] - weights[baseRank - 1]) * progressToNext;
			}

			return {
				scenarioName,
				score: scenarioWeight,
				percentage: (scenarioWeight / maxWeight) * 100
			};
		}
	);
	...
	Object.entries(results).forEach(([categoryName, subcategories]) => {
		...
		Object.values(subcategories)
			.flat()
			.forEach(({ scenarioName, score, percentage }) => {
 ...
 */
function calculateRaS4Rank(
	apiData: BenchmarkApiData,
	difficultyConfig: Difficulty
): RankCalculationResult {
	const isEasy = difficultyConfig.difficultyName.toLowerCase() === 'easy';
	const weights = isEasy ? [20, 50, 80, 110, 140, 170] : [200, 235, 270, 320, 360, 400];
	const rankThresholds = isEasy
		? [240, 600, 960, 1320, 1680, 2040]
		: [2400, 2820, 3240, 3840, 4320, 4800];

	const categoryWeightedScores: number[] = difficultyConfig.categories.map((categoryConfig) => {
		const { topScenarios } = getTopNScoresFromCategory(
			apiData,
			difficultyConfig,
			categoryConfig.categoryName,
			4
		);
		return topScenarios.reduce(
			(sum, scenario) =>
				sum + interpolateValue(scenario.score, scenario.rank_maxes, weights, 3 / 2),
			0
		);
	});
...
function calculateXYZRank(
	...
	for (const [i, rank] of ranks.entries()) {
		...
		const perCategoryAtRank = Object.values(apiData.categories).map(
			(category) => Object.values(category.scenarios).filter((s) => s.scenario_rank >= rank).length
		);
			...
			return {
				...
				details: {
					...
					progressToNextRank: Math.min(
						scenarioRanks.filter((r) => r >= rank).length / totalScenarios,
						1
					)
				}
			};
 ...
 */
export function calculateAplusAltRank(
	apiData: BenchmarkApiData,
	difficultyConfig: Difficulty
): RankCalculationResult {
	if (!apiData.categories) return { rank: 0, details: {} };
	...
	const totalScenarios = Object.values(apiData.categories).reduce(
		(sum, category) => sum + Object.keys(category.scenarios).length,
		0
	);
 ...
 */
function calculateM0narcSHizkuRank(
	apiData: BenchmarkApiData,
	difficultyConfig: Difficulty
): RankCalculationResult {
 ...
 */
export function calculateValEnergyRank(
	apiData: BenchmarkApiData,
	difficultyConfig: Difficulty
): RankCalculationResult {
	...
	const [start, end] = ranges[difficultyConfig.difficultyName.toLowerCase()] ?? [
		0,
...
function calculateXYZ2Rank(
	...
	const categoryRankInfos: Record<
		string,
		{ baseRank: number; progressToNext: number; isMaxed: boolean }[]
	> = {};
	const scenarioNames = getOrderedScenarioNames(apiData, difficultyConfig);
	let scenarioIndex = 0;
	...
	if (
		totalScenarios > 0 &&
		allScores.length === totalScenarios &&
		allScores.every((score) => score === maxPossibleRank)
	) {
		return {
	...
	const allCategoriesHaveThreeMax = Object.values(categoryScores).every(
		(scores) => scores.filter((score) => score === maxPossibleRank).length >= 3
	);
...
/**
 * Calculates Jade Palace Ground rank
 * - Energy based (+100 energy per rank)
 * - Carries over across difficulties
 * - First 3 subcategories: Average of top 3 scenario energies
 * - 4th subcategory: Best scenario energy
 * - Overall: Harmonic mean of subcategory energies
 * - Truncation at each step to avoid rounding up
 */
export function calculateJadePalaceGroundRank(
	apiData: BenchmarkApiData,
	difficultyConfig: Difficulty,
	allDifficulties: Difficulty[]
): RankCalculationResult {
	...
	// Use rank thresholds for actual rank calculation (remove fake upper bound)
	const rankThresholds = energyThresholds.slice(0, -1);
	...
	difficultyConfig.categories.forEach((categoryConfig) => {
		...
		categoryConfig.subcategories.forEach((subcategoryConfig) => {
			...
			// Calculate energy for all scenarios in this subcategory
			for (
				let i = 0;
				i < subcategoryConfig.scenarioCount && scenarioIndex < scenarioNames.length;
				i++, scenarioIndex++
				...
				if (score > 0 && scenarioData.rank_maxes && scenarioData.rank_maxes.length > 0) {
					...
					if (rankInfo.baseRank === 0) {
						// Below first rank: proportional energy to first threshold
						const firstThreshold = rankThresholds[0];
						const firstRankScore = scenarioData.rank_maxes[0];
						if (firstRankScore > 0) {
							const proportion = Math.min(score / firstRankScore, 1);
							energy = proportion * firstThreshold;
						}
					} else if (rankInfo.isMaxed && rankInfo.baseRank === scenarioData.rank_maxes.length) {
						// At or above max rank: allow 1 fake rank above
						// Calculate how far above max rank we are
						const lastRankScore = scenarioData.rank_maxes[scenarioData.rank_maxes.length - 1];
						const secondLastRankScore =
							scenarioData.rank_maxes.length > 1
								? scenarioData.rank_maxes[scenarioData.rank_maxes.length - 2]
								: 0;
						const rankDiff = lastRankScore - secondLastRankScore || 1;

						// Calculate progress beyond max rank
						const extraProgress = (score - lastRankScore) / rankDiff;

						// Cap at 1 full rank above
						const cappedExtraProgress = Math.min(extraProgress, 1.0);

						const lastThreshold = rankThresholds[rankThresholds.length - 1];
						// Assuming 100 energy per rank
						energy = lastThreshold + cappedExtraProgress * 100;
					} else {
						// Between ranks: interpolate
						const lower = rankThresholds[rankInfo.baseRank - 1] || 0;
						const upper =
			...
			scenarioEnergies.sort((a, b) => b - a);

			let subcategoryEnergy = 0;

			if (subcategoryGlobalIndex < 3) {
				// First 3 subcategories: Average of top 3
				const top3 = scenarioEnergies.slice(0, 3);
				if (top3.length > 0) {
					const sum = top3.reduce((a, b) => a + b, 0);
					subcategoryEnergy = sum / top3.length;
				}
			} else {
				// 4th subcategory (and beyond): Best scenario only
				if (scenarioEnergies.length > 0) {
					subcategoryEnergy = scenarioEnergies[0];
				}
			}

			// Truncate subcategory energy
			subcategoryEnergy = Math.trunc(subcategoryEnergy);

			subcategoryEnergies.push(subcategoryEnergy);
			energyDetails[categoryName][subcategoryName] = subcategoryEnergy;
			subcategoryEnergiesFlat.push({ subcategoryName, energy: subcategoryEnergy });

			subcategoryGlobalIndex++;
		});
	});
	...
	if (validEnergies.length > 0) {
		if (validEnergies.length === totalSubcategories) {
			// All subcategories have energy
			const harmonicSum = validEnergies.reduce((sum, e) => sum + 1 / e, 0);
			harmonicMean = totalSubcategories / harmonicSum;
		} else {
			// Some subcategories missing - penalize
			const paddedEnergies = [...subcategoryEnergies];
			for (let i = 0; i < paddedEnergies.length; i++) {
				if (paddedEnergies[i] === 0) {
					paddedEnergies[i] = 0.1; // Small epsilon
				}
			}
			const harmonicSum = paddedEnergies.reduce((sum, e) => sum + 1 / e, 0);
			harmonicMean = totalSubcategories / harmonicSum;
	...
	// Truncate harmonic mean
	harmonicMean = Math.trunc(harmonicMean);
